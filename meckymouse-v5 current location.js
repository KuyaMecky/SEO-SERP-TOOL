...
async function getServerIPInfo() {
  try {
    ...
    console.warn('Failed to get server IP info, using defaults');
    return {
      ip: 'server_ip',
      country: 'Unknown',
      timezone: 'UTC',
...
// Traffic source simulation functions
...
async function generateTraffic(visitId) {
  ...
  const serverInfo = await getServerIPInfo();
  const config = {
  ...
  try {
    ...
    console.log(`📊 Visit ${visitId} completed (${selectedSource}, ${dwellTime}s, IP: ${config.ip})`);
...
// Main execution
(async () => {
  console.log('🚀 Starting server-based traffic generation');
  
  // Get server IP info first
  const serverInfo = await getServerIPInfo();
  console.log(`🌍 Server IP: ${serverInfo.ip} (${serverInfo.country}, ${serverInfo.timezone})`);
