...
// const PROXY_TEST_URL = 'http://httpbin.org/ip';
...
async function getWorkingProxies() {
  ...
  const limit = pLimit(10); // Limit concurrency to avoid API rate-limits
...
async function visitWithProxy(page, url, proxy, config) {
  try {
    ...
    await page.setExtraHTTPHeaders({
      ...
      'Referer': 'https://www.google.com/search?q=chinchin+casino'
    });
